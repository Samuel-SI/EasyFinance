...
class AbaInvestimentos(ctk.CTkFrame):
    def __init__(self, parent, repository, usuario_atual):
        ...
        self.titulo = ctk.CTkLabel(self.frame_esquerda, text="💹 Investimentos Corporativos", font=("Arial", 22, "bold"))
        ...
        ctk.CTkLabel(self.card_usd, text="Dólar (USD)", font=("Arial", 11, "bold"), text_color="#aaa").pack(pady=(8, 2))
        self.lbl_usd_valor = ctk.CTkLabel(self.card_usd, text="Carregando...", font=("Arial", 16, "bold"), text_color="#2ecc71")
        ...
        ctk.CTkLabel(self.card_eur, text="Euro (EUR)", font=("Arial", 11, "bold"), text_color="#aaa").pack(pady=(8, 2))
        self.lbl_eur_valor = ctk.CTkLabel(self.card_eur, text="Carregando...", font=("Arial", 16, "bold"), text_color="#2ecc71")
        ...
        ctk.CTkLabel(self.frame_form, text="Registrar Nova Aquisição", font=("Arial", 13, "bold")).pack(pady=8)

        ctk.CTkLabel(self.frame_form, text="Selecione o Ativo:", font=("Arial", 11), text_color="#aaa").pack(anchor="w", padx=20)
        ...
        ctk.CTkLabel(self.frame_form, text="Preço Unitário do Ativo (R$):", font=("Arial", 11), text_color="#aaa").pack(anchor="w", padx=20)
        ...
        self.lbl_limite_maximo = ctk.CTkLabel(self.frame_form, text="Saldo disponível: Calculando...", font=("Arial", 11, "italic"), text_color="#34D399")
        self.lbl_limite_maximo.pack(anchor="w", padx=20, pady=1)

        ctk.CTkLabel(self.frame_form, text="Quantidade a Comprar:", font=("Arial", 11), text_color="#aaa").pack(anchor="w", padx=20)
        self.txt_qtd = ctk.CTkEntry(self.frame_form, placeholder_text="Ex: 50")
        self.txt_qtd.pack(fill="x", padx=20, pady=(2, 5))

        self.btn_usar_maximo = ctk.CTkButton(
            self.frame_form, text="Preencher Quantidade Máxima", font=("Roboto", 11, "bold"),
        ...
        self.btn_salvar = ctk.CTkButton(
            self.frame_form, text="Confirmar Investimento", 
        ...
        ctk.CTkLabel(self.frame_conversor, text="💱 Conversor de Moedas Comercial (API)", font=("Arial", 13, "bold")).pack(pady=5)
        
        frame_inputs_conv = ctk.CTkFrame(self.frame_conversor, fg_color="transparent")
        frame_inputs_conv.pack(fill="x", padx=20, pady=5)
        
        self.txt_valor_brl = ctk.CTkEntry(frame_inputs_conv, placeholder_text="Valor em Reais (R$)", width=140)
        ...
        self.lbl_resultado_conv = ctk.CTkLabel(self.frame_conversor, text="Resultado: ---", font=("Arial", 12, "bold"), text_color="#6366F1")
        self.lbl_resultado_conv.pack(pady=5)
        
        self.btn_converter = ctk.CTkButton(
            self.frame_conversor, text="Calcular Conversão", font=("Roboto", 11, "bold"),
        ...
        ctk.CTkLabel(self.frame_direita, text="📊 Sua Carteira de Ativos", font=("Arial", 16, "bold")).pack(pady=10)
        ...
        self.btn_rebalancear = ctk.CTkButton(
            self.frame_direita, text="⚖️ Verificar Rebalanceamento (Metas)",
        ...
        self.btn_exportar = ctk.CTkButton(
            self.frame_direita, text="📥 Exportar Relatório Patrimonial",
    ...
    def vincular_preco_api(self, ativo_selecionado):
        ...
        self.lbl_limite_maximo.configure(
            text=f"Saldo: R$ {saldo:.2f} | Máximo permitido: {self.max_permitido:.2f} un.",
    ...
    def definir_quantidade_maxima(self):
        ...
        if not self.cotacoes_atuais or ativo not in self.cotacoes_atuais:
            return messagebox.showwarning("Aviso", "Aguarde o carregamento das cotações da API.")
    ...
    def atualizar_painel(self):
        try:
            ...
            if cotacoes:
                ...
                self.lbl_usd_valor.configure(text="Offline", text_color="#e74c3c")
                self.lbl_eur_valor.configure(text="Offline", text_color="#e74c3c")
        except Exception:
            self.lbl_usd_valor.configure(text="Erro API", text_color="#e74c3c")
    ...
    def executar_conversao_moeda(self):
        ...
        if not self.cotacoes_atuais:
            self.lbl_resultado_conv.configure(text="Erro: Conversor indisponível (Offline)", text_color="#EF4444")
            return
            
        if not valor_brl_str:
            messagebox.showwarning("Aviso", "Insira um valor em Reais para efetuar a conversão.")
            return
        ...
        try:
            ...
            self.lbl_resultado_conv.configure(text=f"Resultado: {resultado:.2f} {moeda_alvo}", text_color="#10B981")
        except ValueError:
            messagebox.showerror("Erro", "Digite apenas caracteres numéricos válidos no conversor.")
    ...
    def calcular_rebalanceamento(self):
        ...
        if not investimentos:
            messagebox.showinfo("Rebalanceamento", "Sua carteira está vazia! Sugestão: Comece aportando igualmente em USD e EUR.")
            return

        if not self.cotacoes_atuais:
            messagebox.showerror("Erro", "Não é possível rebalancear sem acesso às cotações de mercado.")
            return
        ...
        if 45.0 <= percentual_usd <= 55.0:
            messagebox.showinfo(
                "⚖️ Meta Cumprida!",
                f"Sua carteira está perfeitamente balanceada de acordo com as diretrizes!\n\n"
                f"Distribuição Atual:\n"
                f"• Dólar (USD): {percentual_usd:.1f}%\n"
                f"• Euro (EUR): {percentual_eur:.1f}%"
            )
        else:
            # Sugere aporte na moeda que estiver abaixo da linha de 50%
            sugestao = "Dólar (USD)" if percentual_usd < percentual_eur else "Euro (EUR)"
            messagebox.showinfo(
                "⚖️ Sugestão de Rebalanceamento",
                f"Desvio detectado das diretrizes ideais de alocação de risco (50%/50%).\n\n"
                f"Alocação Atual:\n"
                f"• USD: {percentual_usd:.1f}%\n"
                f"• EUR: {percentual_eur:.1f}%\n\n"
                f"💡 Recomendação: Direcione os próximos aportes para adquirir {sugestao}."
            )
    ...
    def cadastrar_ativo(self):
        ...
        if not self.cotacoes_atuais or ticker not in self.cotacoes_atuais:
            messagebox.showerror("Erro", "Ativo ou cotação da API inválidos.")
            return
        ...
        try:
            ...
            if quantidade <= 0:
                messagebox.showwarning("Aviso", "A quantidade precisa ser maior que zero!")
                return

            if total_investimento > saldo_disponivel:
                messagebox.showerror(
                    "Saldo Insuficiente", 
                    f"Você não possui saldo em conta para esta operação!\n\n"
                    f"Custo Total: R$ {total_investimento:.2f}\n"
            ...
            messagebox.showinfo("Sucesso", f"Ativo {ticker} comprado!\n\nValor debitado da conta: R$ {total_investimento:.2f}")
        except ValueError:
            messagebox.showerror("Erro", "Digite apenas números válidos no campo de Quantidade!")
    ...
    def renderizar_carteira(self):
        ...
        if not hasattr(self.usuario_atual, 'investimentos') or not self.usuario_atual.investimentos:
            lbl_vazio = ctk.CTkLabel(self.lista_carteira, text="Nenhum ativo comprado ainda.", text_color="#777", font=("Arial", 12, "italic"))
        ...
        for ativo in self.usuario_atual.investimentos:
            ...
            texto_pago = f"Pago: R$ {ativo['preco_compra']:.2f} (Total: R$ {perf['custo_total']:.2f})"
    ...
    def exportar_relatorio(self):
        ...
        if sucesso:
            enviar_agora = messagebox.askyesno(
                "Relatório Gerado", 
                f"O relatório foi salvo em seu Desktop.\n\nDeseja enviar uma cópia para o e-mail: {getattr(self.usuario_atual, 'email', 'seu-email@provedor.com')}?"
            )
            
            if enviar_agora:
                self.btn_exportar.configure(text="📨 Enviando E-mail...", state="disabled")
                self.update_idletasks()
                
                email_ok, msg_email = self.service.enviar_relatorio_por_email(self.usuario_atual, caminho_arquivo)
                self.btn_exportar.configure(text="📥 Exportar Relatório Patrimonial", state="normal")
                
                if email_ok:
                    messagebox.showinfo("Sucesso!", f"Relatório enviado com sucesso para {getattr(self.usuario_atual, 'email', 'seu-mail')}!")
                else:
                    messagebox.showerror("Erro de Envio", f"O arquivo foi salvo localmente, mas não pôde ser enviado por e-mail:\n\n{msg_email}")
            else:
                messagebox.showinfo("Concluído", f"O relatório está disponível em:\n{caminho_arquivo}")
        else:
            messagebox.showwarning("Erro de Exportação", caminho_arquivo)